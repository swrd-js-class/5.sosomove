        ...
        <div class="bg-white py-6 px-10 border">
          ...
          <form onSubmit={handleSubmit}>
            <div class="space-y-4 mt-6 mb-10">
              ...
              <div class="w-full">
                <h2>대표자명</h2>
                <input type="text" value={ceo_name} onChange={(e) => setCeoName(e.target.value)} class="px-4 py-2 bg-gray-50 rounded-lg" />
              </div>
              <div class="w-full">
                <h2>사업장 주소</h2>
                <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} class="px-4 py-2 bg-gray-50 rounded-lg" />
              </div>
            </div>
            <button type="submit" class="w-full mt-5 bg-indigo-600 text-white py-2 rounded-lg font-semibold tracking-tight">수정</button>
          </form>
          <button onClick={deleteAccount} class="w-full mt-5 bg-red-600 text-white py-2 rounded-lg font-semibold tracking-tight">탈퇴</button>
        </div>
      </div>
    </div>
  );
