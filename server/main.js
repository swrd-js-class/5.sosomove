...
//   // 이미 사용된 request_id를 추적하기 위한 배열
//   const usedRequestIds = CollectionEstCar.find().map(car => car.request_id);

//   for (let i = 0; i < 2; i++) {
//     // 사용되지 않은 request_id 찾기
//     const availableRequests = requests.filter(request => !usedRequestIds.includes(request._id));
    
//     if (availableRequests.length === 0) {
//       break;  // 더 이상 사용되지 않은 request_id가 없다면 종료
//     }
...
//       furniture: ["침대메트리스", "침대프레임", "책상", "의자", "소파", "테이블", "수납장", "서랍장", "책장", "옷장", "화장대", "행거"].shuffle().slice(0, 2),
//       detail: "",
//       picture: [],
//     });

//     // 사용된 request_id를 추적
//     usedRequestIds.push(randomRequest._id);
//   }
// }
